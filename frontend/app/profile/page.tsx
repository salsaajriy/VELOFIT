...
export default function ProfilePage() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (profileData.weight && profileData.height) {
        const weightNum =
          typeof profileData.weight === "string"
            ? parseFloat(profileData.weight)
            : profileData.weight;
        const heightNum =
          typeof profileData.height === "string"
  ...
  const calculateBMI = (weight: number, height: number) => {
    ...
    if (bmiValue < 18.5) {
      ...
      color = "#3b82f6";
      advice =
        "You are below the healthy weight range. Consider increasing calorie intake with nutritious foods and consult a nutritionist.";
      ...
      color = "#22c55e";
      advice =
        "Great! You are in the healthy weight range. Maintain this with balanced diet and regular exercise.";
      const minIdeal = 18.5 * (heightInMeters * heightInMeters);
      const maxIdeal = 24.9 * (heightInMeters * heightInMeters);
      idealWeightRange = `${minIdeal.toFixed(1)} - ${maxIdeal.toFixed(1)} kg`;
      ...
      color = "#f59e0b";
      advice =
        "You are slightly above the healthy weight range. Small changes in diet and increased physical activity can help.";
      const minIdeal = 18.5 * (heightInMeters * heightInMeters);
      const maxIdeal = 24.9 * (heightInMeters * heightInMeters);
      idealWeightRange = `${minIdeal.toFixed(1)} - ${maxIdeal.toFixed(1)} kg`;
      ...
      color = "#ef4444";
      advice =
        "You are in the obese category. We strongly recommend consulting a healthcare provider for a personalized weight management plan.";
      const minIdeal = 18.5 * (heightInMeters * heightInMeters);
      const maxIdeal = 24.9 * (heightInMeters * heightInMeters);
      idealWeightRange = `${minIdeal.toFixed(1)} - ${maxIdeal.toFixed(1)} kg`;
    } else if (bmiValue >= 30) {
      ...
      color = "#dc2626";
      advice =
        "You are in the severely obese category. Please consult a healthcare professional immediately for proper medical guidance.";
      const minIdeal = 18.5 * (heightInMeters * heightInMeters);
      const maxIdeal = 24.9 * (heightInMeters * heightInMeters);
      idealWeightRange = `${minIdeal.toFixed(1)} - ${maxIdeal.toFixed(1)} kg`;
    }
  ...
  const bmiProgress =
    bmiData.bmi > 0
      ? Math.min(Math.max(((bmiData.bmi - 10) / 30) * 100, 0), 100)
      : 0;
