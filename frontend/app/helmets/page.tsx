  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex items-center gap-0.5">
        ...
        <div
          className="w-1 h-2 rounded-r-sm ml-0.5"
...
function PairModal({
  ...
  const scanBLE = async () => {
    ...
    if (!nav.bluetooth) {
      onToast("Bluetooth Web API is not supported in this browser.", "error");
      return;
    }
    ...
    try {
      ...
      if (device.name) {
        ...
        onToast(`Connected to ${device.name}`, "success");
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      onToast(
        err instanceof Error ? err.message : "Failed to pair helmet.",
        "error",
      );
    } finally {
      setLoading(false);
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2 pt-1">
            <button
              ...
              disabled={loading || !helmetName.trim() || !bleConnected}
              ...
              style={{
                background: "linear-gradient(135deg, #c2440a, #b83208)",
              }}
            >
...
function HelmetCard({
  ...
  const connection: ConnectionStatus = helmet.lastSeen
    ? new Date().getTime() - new Date(helmet.lastSeen).getTime() < 30000
      ? "connected"
      : "offline"
    : "offline";
  ...
  return (
    <div className="group relative bg-white rounded-2xl border-2 border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all duration-200 overflow-hidden">
      <div className="p-5">
            ...
            <div className="flex items-center gap-1.5 mb-0.5 pr-16">
              {editingName ? (
                ...
                <>
                  ...
                  <button
                    onClick={() => {
                      setEditName(helmet.helmet_name);
                      setEditingName(true);
                    }}
...
export default function ManageHelmetsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (mounted) setHelmets(helmetsWithStatus);
      } catch (error) {
  ...
  const handleRename = async (id: number, name: string) => {
    try {
      ...
      showToast(
        err instanceof Error ? err.message : "Failed to update helmet name.",
        "error",
      );
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      showToast(
        err instanceof Error ? err.message : "Failed to unpair helmet.",
        "error",
      );
    }
  };
  ...
  const handleBLEConnect = async () => {
    try {
      ...
      showToast(
        err instanceof Error ? err.message : "BLE connection failed.",
        "error",
      );
    }
  };
            ...
            {loading ? (
              ...
              <div className="bg-white rounded-2xl border-2 border-dashed border-gray-200 p-14 flex flex-col items-center text-center">
                ...
                <p className="text-sm font-bold text-gray-500 mb-1">
                  No Registered Helmet
                </p>
                <p className="text-xs text-gray-400 mb-5">
                  Start by pairing your helmet to view battery level and
                  connection status.
                </p>
                <button
                  onClick={() => setShowPairModal(true)}
